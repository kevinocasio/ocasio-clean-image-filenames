<?php
/**
 * Plugin Name: Ocasio Clean Image Filenames
 * Plugin URI:  https://kevinocasio.com/wordpress-plugins/ocasio-clean-image-filenames/
 * Description: Automatically sanitizes uploaded media file names, converting spaces to hyphens and removing special characters for better SEO.
 * Version:     1.0.0
 * Author:      Kevin Ocasio
 * Author URI:  https://kevinocasio.com
 * License:     GPL-2.0-or-later
 * License URI: https://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain: ocasio-clean-image-filenames
 */
...
define('OCASIO_CIF_VERSION', '1.0.0');
define('OCASIO_CIF_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('OCASIO_CIF_PLUGIN_URL', plugin_dir_url(__FILE__));

/**
 * Helper: Smart Brand URL Resolver (Keeps links local on .local, points to live domain on production)
 */
function ocasio_cif_brand_url($path = '/wordpress-plugins/') {
 ...
 */
function ocasio_cif_activate() {
	if (get_option('ocasio_cif_enabled') === false) {
		$legacy = get_option('ko_cif_enabled', 1);
		update_option('ocasio_cif_enabled', (int) $legacy);
	}
}
register_activation_hook(__FILE__, 'ocasio_cif_activate');

/**
 * 2. Core Logic: Sanitize Uploaded Filenames
 */
function ocasio_cif_sanitize_filename($filename) {
	$enabled = get_option('ocasio_cif_enabled');
	if ($enabled === false) {
		$enabled = get_option('ko_cif_enabled', 1);
	}
	...
	// 4. Remove all non-alphanumeric characters except hyphen
	$escaped_separator = preg_quote($separator, '/');
	$name = preg_replace('/[^a-z0-9' . $escaped_separator . ']/', '', $name);

	// 5. Remove multiple consecutive hyphens
	$name = preg_replace('/' . $escaped_separator . '+/', $separator, $name);
...
add_filter('sanitize_file_name', 'ocasio_cif_sanitize_filename', 10);

/**
 * 3. Register Settings
 */
function ocasio_cif_register_settings() {
	register_setting('ocasio_cif_settings_group', 'ocasio_cif_enabled', array(
		'type'              => 'integer',
		'sanitize_callback' => 'absint',
		'default'           => 1,
	));
}
add_action('admin_init', 'ocasio_cif_register_settings');

/**
 * 4. Register Admin Menu under Ocasio Plugins (Position 65)
 */
function ocasio_cif_register_menu() {
...
add_action('admin_menu', 'ocasio_cif_register_menu');

/**
 * 5. Action Link on Plugins Screen
 */
function ocasio_cif_action_links($links) {
	$settings_link = '<a href="' . esc_url(admin_url('admin.php?page=ocasio-plugins-main')) . '">' . esc_html__('Dashboard', 'ocasio-clean-image-filenames') . '</a>';
	array_unshift($links, $settings_link);
	return $links;
}
add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'ocasio_cif_action_links');

/**
 * 6. Conditional Admin Asset Enqueuing
 */
function ocasio_cif_admin_assets($hook) {
	...
	if (strpos($hook, 'ocasio-plugins-main') !== false || $hook === 'toplevel_page_ocasio-plugins-main') {
		wp_enqueue_style('ocasio-cif-admin-css', plugins_url('assets/admin.css', __FILE__), array(), OCASIO_CIF_VERSION);
		wp_enqueue_script('ocasio-cif-admin-js', plugins_url('assets/admin.js', __FILE__), array('jquery'), OCASIO_CIF_VERSION, true);
		wp_localize_script('ocasio-cif-admin-js', 'ocasio_vars', array(
			'ajaxurl'     => admin_url('admin-ajax.php'),
			'suite_nonce' => wp_create_nonce('ocasio_suite_toggle_nonce'),
		));
		wp_localize_script('ocasio-cif-admin-js', 'ocasio_301_vars', array(
			'ajaxurl'     => admin_url('admin-ajax.php'),
			'suite_nonce' => wp_create_nonce('ocasio_suite_toggle_nonce'),
		));
	}
}
add_action('admin_enqueue_scripts', 'ocasio_cif_admin_assets');
